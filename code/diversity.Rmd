...
rownames(phylo.dist.m) <- trimws(rownames(phylo.dist.m))

# Check order of design
all.equal(row.names(eDNA.div), row.names(phylo.dist.m))

# Split into two and rearrange
...
# Centroid Distances Calculations
## Taxonomic Centroid Distance Ratios
```{r}
# Data Check
all.equal(row.names(eDNA.div), row.names(OTU.REL.log))
...
## Phylogenetic Centroid Distance Ratios
```{r}
# Data Check
all.equal(row.names(eDNA.div), row.names(phylo.dist.m))
...
# Bray Curtis Plot
cent.dist.table <- t.cent.dist.table
centroid.plot <- plot(cent.dist.table$mean, ylim = c(0.6, 1.6), 
      xlim = c(0.5, 4.5), pch = 22, bg = "white", lwd = 2, 
      cex = 3, yaxt = "n", xaxt = "n", cex.lab = 2, cex.axis = 1.5, 
      las = 1, ylab = "", xlab = "")
      box(lwd = 2)
