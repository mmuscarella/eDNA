...
# # Good's coverage --> I think function is caculating singletons for entire matrix
# goods.c <- function(x = ""){
#   1 - (sum(x == 1) / rowSums(x))
# }
# 
# goods.c.eDNA <- goods.c(OTU)
# mean.good.c <- mean(goods.c.eDNA)
# min.good.c <- min(goods.c.eDNA)

# eDNA_57, eDNA_23, eDNA_24 are negative??? Samples have lowest seqs
```
...
even.sem <- round(apply(even.data[8:1007], 1, sem, na.rm = TRUE),3) # returns zeros?
...
# PERMANOVA
```{r}
eDNA.permanova <- adonis(OTU.REL ~ design$env + design$treat + design$env * design$treat, 
    strata = design$pair, method = "bray", binary = TRUE)
```
