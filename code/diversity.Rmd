...
# Reshape data
rich.reshape <- reshape(rich.rep.ave[,1:4], timevar = "treat", 
                   idvar = c("qPCR.match", "env"), direction = "wide")

even.reshape <- reshape(even.rep.ave[,1:4], timevar = "treat", 
                   idvar = c("qPCR.match", "env"), direction = "wide")
```
...
```{r}
# Calculate ratios
rich.mean.ratio <- data.frame(rich.reshape, (ratio.1$mean.E / ratio.1$mean.C))
colnames(rich.mean.ratio) <- c("env","qPCR.match", "rich.C", "rich.E", "rich.ratio")
rich.mean.ratio.ag <- aggregate(rich.ratio ~ env + qPCR.match, rich.mean.ratio, mean)

# Richness table
rich.mean.ratio.ag.tab <- aggregate(rich.ratio ~ env, rich.mean.ratio, mean)
rich.sem.ratio.ag.tab <- aggregate(rich.ratio ~ env, rich.mean.ratio, mean)
rich.95.ratio.ag.tab <- aggregate(rich.ratio ~ env, rich.mean.ratio, 
                  FUN = function(x) t.test(x)$conf.int[1:2])
rich.table <- data.frame(rich.mean.ratio.ag.tab, rich.sem.ratio.ag.tab[,2], 
                  rich.95.ratio.ag.tab[,2])
...
sample.list <- paste(as.character(rich.mean.ratio$qPCR.match), collapse=", ")
print(sample.list)
eDNA.prop.sub<-eDNA.prop[eDNA.prop$sample.number %in% c(30, 31, 33, 34, 2, 7, 11, 
                  13, 15, 16, 5, 6, 26, 27, 29, 3, 10, 12, 14, 17, 20),]
eDNA.prop.sub <- eDNA.prop.sub[order(eDNA.prop.sub$sample.number),]
rich.reg <- lm(rich.mean.ratio.ag$rich.ratio ~ eDNA.prop.sub$prop)
summary(rich.reg)
...
```{r}
# Calculate ratios
even.mean.ratio <- data.frame(even.reshape, (ratio.1$mean.E / ratio.1$mean.C))
colnames(even.mean.ratio) <- c("env","qPCR.match", "even.C", "even.E", "even.ratio")
even.mean.ratio.ag <- aggregate(even.ratio ~ env + qPCR.match, even.mean.ratio, mean)

# Evennes table
even.mean.ratio.ag.tab <- aggregate(even.ratio ~ env, even.mean.ratio, mean)
even.sem.ratio.ag.tab <- aggregate(even.ratio ~ env, even.mean.ratio, mean)
even.95.ratio.ag.tab <- aggregate(even.ratio ~ env, even.mean.ratio, 
                  FUN = function(x) t.test(x)$conf.int[1:2])
even.table <- data.frame(even.mean.ratio.ag.tab, even.sem.ratio.ag.tab[,2], 
                  even.95.ratio.ag.tab[,2])
colnames(even.table) <- c("env", "mean", "sem", "LCI", "UCI")

# Regression: evennes ratio vs. proportion eDNA
eDNA.prop.sub <- eDNA.prop.sub[order(eDNA.prop.sub$sample.number),]
even.reg <- lm(even.mean.ratio.ag$even.ratio ~ eDNA.prop.sub$prop)
summary(even.reg)
