...
# Phylo: differences among sites?
```{r}
faith.anova <- aov(even.2$even.C ~even.2$env)
summary(even.anova)
TukeyHSD(even.anova)
```
...
eDNA.pairs$prop <- eDNA.prop$prop[match(eDNA.prop$sample.number, eDNA.pairs$qPCR)]
...
cent.dist.table <- as.data.frame(cent.dist.table)

```


# Centoid Distance Ratio Plot
```{r}
...
explainvar1 <- round(eDNA.phylo.PCoA$eig[1] / sum(eDNA.phylo.PCoA$eig), 3) * 100
explainvar2 <- round(eDNA.phylo.PCoA$eig[2] / sum(eDNA.phylo.PCoA$eig), 3) * 100
explainvar3 <- round(eDNA.phylo.PCoA$eig[3] / sum(eDNA.phylo.PCoA$eig), 3) * 100
sum.eig <- sum(explainvar1, explainvar2, explainvar3)
...
dist.c <- phylo.dist.m[which(eDNA.div$treat == "C"), which(eDNA.div$treat == "C")]
dist.e <- phylo.dist.m[which(eDNA.div$treat == "E"), which(eDNA.div$treat == "E")]
