...
# Distances between pairs (Distance Ratio): Taxonomic
```{r}
# Data Check
all.equal(row.names(eDNA.div), row.names(OTU.REL.log))
...
for (i in 1:length(pair.names)){
  ...
  eDNA.pairs$env[i] <- eDNA.div$env[which(colnames(dist.raw) == samp.temp[1])]
  eDNA.pairs$dis[i] <- dist.temp
  eDNA.pairs$qPCR[i] <- eDNA.div$qPCR.match[which(colnames(dist.raw) == samp.temp[1])]
}
...
pair.dis.95 <- aggregate(dis ~ env, eDNA.pairs, 
                  FUN = function(x) t.test(x)$conf.int[1:2])
pair.dis.table <- data.frame(env = pair.dis.mean$env, mean = pair.dis.mean$dis,
                         sem = pair.dis.sem$dis, ci = pair.dis.95$dis)
colnames(pair.dis.table) <- c("env", "mean", "sem", "LCI", "UCI")
```
...
# Delta Distances
## Taxonomic Delta Distances (PCoA)
```{r}
...
# Pair Distance as funciton of proportion eDNA
```{r}
eDNA.pairs
eDNA.prop


eDNA.pairs$prop <- eDNA.prop.sub2$prop[match(eDNA.prop.sub2$sample.number, eDNA.pairs$pair)]
