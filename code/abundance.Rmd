...
# eDNA.raw[,7] = copies not corrected by dilutioin factor
...
# Mean of Technical Reps Taking into Account Pairs

eDNA.2 <- reshape(eDNA.corr[,c(3,4,5,6,11)], timevar = "treat", idvar = c("sample_name", "env", "rep"), direction = "wide")
eDNA.2$prop <- (1 - (eDNA.2$copies.corr.E / eDNA.2$copies.corr.C))
eDNA.3 <- eDNA.2[-which(eDNA.2$prop < 0), ]
eDNA.4 <- aggregate(prop ~ sample_name + env, eDNA.3, mean)
eDNA.mean <- aggregate(prop ~ env, eDNA.4, mean)
eDNA.se <- aggregate(prop ~ env, eDNA.4, se)
...
# Make plot with error bars by environment
```{r}
...
# Show Plot
img <- readPNG("figures/qPCR.nonbar.png")
 grid.raster(img)
```