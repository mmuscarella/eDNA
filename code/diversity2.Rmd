...
# Calculate Taxonomic Centroid Distances
soil.cent <- centroid.dist(env = "soil" , sbys = OTU.REL.log, design = eDNA.div)
sed.cent <- centroid.dist(env = "sed" , sbys = OTU.REL.log, design = eDNA.div)
feces.cent <- centroid.dist(env = "feces" , sbys = OTU.REL.log, design = eDNA.div)
water.cent <- centroid.dist(env = "water" , sbys = OTU.REL.log, design = eDNA.div)

centroid.distances.tax <- rbind(feces.cent, sed.cent, soil.cent, water.cent)

# Calculate Phylogenetic Centroid Distances
soil.cent <- centroid.dist(env = "soil" , sbys = OTU.REL.log, 
                           design = eDNA.div, phylo = phylo.dist)
sed.cent <- centroid.dist(env = "sed" , sbys = OTU.REL.log, 
                          design = eDNA.div, phylo = phylo.dist)
feces.cent <- centroid.dist(env = "feces" , sbys = OTU.REL.log, 
                            design = eDNA.div, phylo = phylo.dist)
water.cent <- centroid.dist(env = "water" , sbys = OTU.REL.log, 
                            design = eDNA.div, phylo = phylo.dist)

centroid.distances.phylo <- rbind(feces.cent, sed.cent, soil.cent, water.cent)


# We shouldn't need anything below here. 
# Well we need to get the code for the stats and linear models.
...
# Soil
env.soil <- which(eDNA.env == "soil")
cent.soil <- as.data.frame(matrix(NA, nrow = length(env.soil), ncol = 3))
...
# Ratio Data Table
ratio.list <- list(feces = cent.feces.ratio[,1], sed = cent.sed.ratio[,1],
                     soil = cent.soil.ratio[,1], water = cent.water.ratio[,1])
...
cent.tax.dist.table$mean <- unlist(lapply(ratio.list, mean))
cent.tax.dist.table$sem <- unlist(lapply(ratio.list, sem))
cent.tax.dist.table$LCI <- unlist(lapply(ratio.list, FUN = function(x) t.test(x)$conf.int[1]))
cent.tax.dist.table$UCI <- unlist(lapply(ratio.list, FUN = function(x) t.test(x)$conf.int[2]))
t.cent.tax.dist.table <- as.data.frame(cent.tax.dist.table)
cent.tax.dist.order <- c("Soil", "Sediment", "Gut", "Water")
t.cent.tax.dist.table <- t.cent.tax.dist.table[match(cent.tax.dist.order, t.cent.tax.dist.table$env),]

# Regression: centroid ratio vs. proportion eDNA
# Combine Centroid Distances
centroid.tax.dists <- c(cent.feces.ratio$T, cent.sed.ratio$T, cent.soil.ratio$T,
                        cent.water.ratio$T)
names(centroid.tax.dists) <- c(rownames(cent.feces.ratio), rownames(cent.sed.ratio),
                           rownames(cent.soil.ratio), rownames(cent.water.ratio))

# Reorganize
names(centroid.tax.dists) %in% paste(eDNA.div.c$env, eDNA.div.c$pair, sep = "")
all.equal(names(centroid.tax.dists), paste(eDNA.div.c$env, eDNA.div.c$pair, sep = ""))
cent.tax.dist <- centroid.tax.dists[match(names(centroid.tax.dists),
                      paste(eDNA.div.c$env, eDNA.div.c$pair, sep = ""))]

# Combine Centroid Distance and Design Information
centroid.tax.div <- cbind(eDNA.div.c[,c(1:3, 5:7)], cent.tax.dist)

centroid.tax.div$qPCR.match %in% eDNA.prop$sample.number

### Something wrong here
centroid.tax.div$prop <- NA
for (i in 1:length(centroid.tax.div$prop)){
  centroid.tax.div$prop[i] <- eDNA.prop$prop[which(eDNA.prop$sample.number ==
                                                 centroid.tax.div$qPCR.match[i])]
}
###

# Take averages of technial reps
centroid.tax.div.t <- ddply(centroid.tax.div, .(env, qPCR.match, prop),
                      summarize, cent.tax.dist = mean(cent.tax.dist, na.rm = TRUE))

t.cent.tax.reg <- lm(cent.tax.dist ~ prop * env, data = centroid.tax.div.t)
t.cent.tax.reg <- lm(cent.tax.dist ~ prop, data = centroid.tax.div.t)

summary(t.cent.tax.reg)
ttest(t.cent.tax.reg, 1, 1)

# answer => no correlation between proportion eDNA and centroid dist ratio.


# One-sample t-tets: taxonomic centroid ratio by ecosystem type

cent.tax.feces <- centroid.tax.div.t[ which(centroid.tax.div.t$env == "feces"),]
cent.tax.feces.ttest <- t.test(cent.tax.feces$cent.tax.dist, mu = 1)

cent.tax.sed <- centroid.tax.div.t[ which(centroid.tax.div.t$env == "sed"),]
cent.tax.sed.ttest <- t.test(cent.tax.sed$cent.tax.dist, mu = 1)

cent.tax.soil <- centroid.tax.div.t[ which(centroid.tax.div.t$env == "soil"),]
cent.tax.soil.ttest <- t.test(cent.tax.soil$cent.tax.dist, mu = 1)

cent.tax.water <- centroid.tax.div.t[ which(centroid.tax.div.t$env == "water"),]
cent.tax.water.ttest <- t.test(cent.tax.water$cent.tax.dist, mu = 1)
```
...
# Ratio Data Table
ratio.phylo.list <- list(cent.feces.ratio[,1], cent.sed.ratio[,1],
                     cent.soil.ratio[,1], cent.water.ratio[,1])
cent.phylo.dist.table <- as.data.frame(matrix(NA, nrow = 4, ncol = 5))
colnames(cent.phylo.dist.table) <- c("env", "mean", "sem", "LCI", "UCI")
cent.phylo.dist.table$env <- c("Gut", "Sediment", "Soil", "Water")
cent.phylo.dist.table$mean <- unlist(lapply(ratio.phylo.list, mean))
cent.phylo.dist.table$sem <- unlist(lapply(ratio.phylo.list, sem))
cent.phylo.dist.table$LCI <- unlist(lapply(ratio.phylo.list, FUN = function(x) t.test(x)$conf.int[1]))
cent.phylo.dist.table$UCI <- unlist(lapply(ratio.phylo.list, FUN = function(x) t.test(x)$conf.int[2]))
p.cent.phylo.dist.table <- as.data.frame(cent.phylo.dist.table)

# Regression: centroid ratio vs. proportion eDNA
# Combine Centroid Distances
centroid.phylo.dists <- c(cent.feces.ratio$T, cent.sed.ratio$T, cent.soil.ratio$T,
                        cent.water.ratio$T)
names(centroid.phylo.dists) <- c(rownames(cent.feces.ratio), rownames(cent.sed.ratio),
                           rownames(cent.soil.ratio), rownames(cent.water.ratio))

# Reorganize
names(centroid.phylo.dists) %in% paste(eDNA.div.c$env, eDNA.div.c$pair, sep = "")
all.equal(names(centroid.phylo.dists), paste(eDNA.div.c$env, eDNA.div.c$pair, sep = ""))
cent.phylo.dist <- centroid.phylo.dists[match(names(centroid.phylo.dists),
                      paste(eDNA.div.c$env, eDNA.div.c$pair, sep = ""))]

# Combine Centroid Distance and Design Information
centroid.phylo.div <- cbind(eDNA.div.c[,c(1:3, 5:7)], cent.phylo.dist)

centroid.phylo.div$qPCR.match %in% eDNA.prop$sample.number

### Something wrong here
centroid.phylo.div$prop <- NA
for (i in 1:length(centroid.phylo.div$prop)){
  centroid.phylo.div$prop[i] <- eDNA.prop$prop[which(eDNA.prop$sample.number ==
                                                 centroid.phylo.div$qPCR.match[i])]
}
####

# Take averages of technial reps
centroid.phylo.div.p <- ddply(centroid.phylo.div, .(env, qPCR.match, prop),
                      summarize, cent.phylo.dist = mean(cent.phylo.dist))

p.cent.phylo.reg <- lm(cent.phylo.dist ~ prop * env, data = centroid.phylo.div.p)
p.cent.phylo.reg <- lm(cent.phylo.dist ~ prop, data = centroid.phylo.div.p)
summary(p.cent.phylo.reg)
ttest(p.cent.phylo.reg, 1, 1)

# answer => no correlation between proportion eDNA and centroid dist ratio.

# One-sample t-tets: phylo centroid ratio by ecosystem type

cent.phylo.feces <- centroid.phylo.div.p[ which(centroid.phylo.div.p$env == "feces"),]
cent.phylo.feces.ttest <- t.test(cent.phylo.feces$cent.phylo.dist, mu = 1)

cent.phylo.sed <- centroid.phylo.div.p[ which(centroid.phylo.div.p$env == "sed"),]
cent.phylo.sed.ttest <- t.test(cent.phylo.sed$cent.phylo.dist, mu = 1)

cent.phylo.soil <- centroid.phylo.div.p[ which(centroid.phylo.div.p$env == "soil"),]
cent.phylo.soil.ttest <- t.test(cent.phylo.soil$cent.phylo.dist, mu = 1)

cent.phylo.water <- centroid.phylo.div.p[ which(centroid.phylo.div.p$env == "water"),]
cent.phylo.water.ttest <- t.test(cent.phylo.water$cent.phylo.dist, mu = 1)
```

# Beta Plots
## Centroid Distance
```{r}
png(filename="../figures/Figure4-Centroid2.png",
...
# Bray Curtis Plot
centroid.tax.plot <- plot(0, 0, type = "n",
...
arrows(x0 = c(1,2,3,4), y0 = t.cent.tax.dist.table$mean, 
       y1 = t.cent.tax.dist.table$LCI, angle = 90, length = 0.1, lwd = 2)
arrows(x0 = c(1,2,3,4), y0 = t.cent.tax.dist.table$mean, 
       y1 = t.cent.tax.dist.table$UCI, angle = 90, length = 0.1, lwd = 2)
points(c(1,2,3,4), t.cent.tax.dist.table$mean, pch = 21, col = "black",
       bg = "white", lwd = 2, cex = 3.5)

points(jitter(rep(1, length(ratio.list$soil)), amount = 0.1), ratio.list$soil, 
       pch = 21, bg = "lightgrey", col = "lightgrey", lwd = 2, cex = 1.7)
points(jitter(rep(2, length(ratio.list$sed)), amount = 0.1), ratio.list$sed, 
       pch = 21, bg = "lightgrey", col = "lightgrey", lwd = 2, cex = 1.7)
points(jitter(rep(3, length(ratio.list$feces)), amount = 0.1), ratio.list$feces,
       pch = 21, bg = "lightgrey", col = "lightgrey", lwd = 2, cex = 1.7)
points(jitter(rep(4, length(ratio.list$water)), amount = 0.1), ratio.list$water, 
       pch = 21, bg = "lightgrey", col = "lightgrey", lwd = 2, cex = 1.7)
...
points(1, mean(cent.tax.soil$cent.tax.dist), pch = 21, col = "black",
       bg = "NA", lwd = 2, cex = 3.5)
points(2, mean(cent.tax.sed$cent.tax.dist), pch = 21, col = "black", 
       bg = "NA", lwd = 2, cex = 3.5)  
points(3, mean(cent.tax.feces$cent.tax.dist), pch = 21, col = "black", 
       bg = "NA", lwd = 2, cex = 3.5)  
points(4, mean(cent.tax.water$cent.tax.dist), pch = 21, col = "black", 
       bg = "NA", lwd = 2, cex = 3.5) 

points(jitter(rep(2, length(cent.tax.sed$cent.tax.dist)), amount = 0.1),
       cent.tax.sed$cent.tax.dist, 
       pch = 21, bg = "lightgrey", col = "lightgrey", lwd = 2, cex = 1.7)
points(jitter(rep(3, length(cent.tax.feces$cent.tax.dist)), amount = 0.1),
       cent.tax.feces$cent.tax.dist, 
       pch = 21, bg = "lightgrey", col = "lightgrey", lwd = 2, cex = 1.7)
points(jitter(rep(4, length(cent.tax.water$cent.tax.dist)), amount = 0.1),
       cent.tax.water$cent.tax.dist, 
       pch = 21, col = "lightgrey", bg = "lightgrey", lwd = 2, cex = 1.7)
...
plot(centroid.div.t$prop, centroid.div.t$cent.dist,
     xlim = c(0, 1), ylim = c(0.5, 2),
     pch = 22, bg = "gray", lwd = 2, type = "n",
     cex = 1.5, yaxt = "n", xaxt = "n", cex.lab = 2, cex.axis = 1.5,
     las = 1, ylab = "", xlab = "")
box(lwd = 2)

pred.frame <- data.frame(prop = seq(0.0, 0.875, length.out = 21))
add.hull(t.cent.reg, pred.frame)

points(centroid.div.t$prop, centroid.div.t$cent.dist,
       pch = 22, bg = "gray", lwd = 2, cex = 2)
...
clip(x1 = 0, x2 = 0.85, y1 = -5, y2 = 2)
abline(t.cent.reg, lty = 2, lwd = 3, col = "gray10")
...
pred.frame <- data.frame(prop = seq(0.0, 0.875, length.out = 21))
add.hull(p.cent.reg, pred.frame)

points(centroid.div.p$prop, centroid.div.p$cent.dist,
       pch = 22, bg = "gray", lwd = 2, cex = 2)
...
clip(x1 = 0, x2 = 0.85, y1 = -5, y2 = 2)
abline(p.cent.reg, lty = 2, lwd = 3, col = "gray10")
