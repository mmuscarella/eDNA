...
Analysis of community sequence data to test whether the structure and composition of bacterial communities is affected by relic DNA.
...
rich.mean <- round(apply(rich.data[8:(7 + dim(rich2)[2])], 1, mean, 
                         na.rm = TRUE),3)
rich.sem <- round(apply(rich.data[8:(7 + dim(rich2)[2])], 1, sem, 
                        na.rm = TRUE), 3)
...
even.mean <- round(apply(even.data[8:(7 + dim(even2)[2])], 1, mean, 
                         na.rm = TRUE),3)
even.sem <- round(apply(even.data[8:(7 + dim(even2)[2])], 1, sem, 
                        na.rm = TRUE),4)

# Make new dataframe merging design file and mean diversity
eDNA.div <- data.frame(design[sort(row.names(design)), ], 
                       rich.mean, even.mean)

# Take averages of technial reps
rich.rep.ave <- ddply(eDNA.div, .(env, treat, qPCR.match), summarize, 
                      rich = mean(rich.mean))
even.rep.ave <- ddply(eDNA.div, .(env, treat, qPCR.match), summarize, 
                      even = mean(even.mean))
...
rich.reg <- lm(rich.2[order(rich.2$qPCR.match), ]$rich.ratio ~ 
                 eDNA.prop.sub2$env + eDNA.prop.sub2$prop + 
                 eDNA.prop.sub2$env*eDNA.prop.sub2$prop)
rich.reg <- lm(rich.2[order(rich.2$qPCR.match), ]$rich.ratio ~ prop, 
               data = eDNA.prop.sub2)
...
even.reg <- lm(even.2[order(even.2$qPCR.match), ]$even.ratio ~
...
sum(colnames(OTU) %in% OTU.tre$tip.label) == 
  length(colnames(OTU) %in% OTU.tre$tip.label)
...
pair.div.tax <- unique(eDNA.div$pair)
pair.dis.tax <- rep(NA, length(pair.div.tax))
for(i in 1:length(pair.div.tax)){
  temp <- row.names(eDNA.div[eDNA.div$pair == pair.div.tax[i], ])
  pair.dis.tax[i] <- eDNA.bc.dis.m[temp[1], temp[2]]
}
...
reps.tax <- names(which(table(eDNA.div$qPCR.match) == 4))
reps.dis.tax <- rep(NA, length(reps.tax))

for(i in 1:length(reps.tax)){
  temp <- eDNA.div[eDNA.div$qPCR.match == reps.tax[i] & 
                     eDNA.div$treat == "C", ]
  temp.names <- row.names(temp)
  reps.dis.tax[i] <- eDNA.bc.dis.m[temp.names[1], temp.names[2]]
}

mean(reps.dis.tax)
sem(reps.dis.tax)
t.test(pair.dis.tax, reps.dis.tax)
```
...
pair.div.phy <- unique(eDNA.div$pair)
pair.dis.phy <- rep(NA, length(pair.div.phy))
for(i in 1:length(pair.div.phy)){
  temp <- row.names(eDNA.div[eDNA.div$pair == pair.div.phy[i], ])
  pair.dis.phy[i] <- eDNA.phylo.m[temp[1], temp[2]]
}
...
# Average BC Distance Between Replicates
reps.phy <- names(which(table(eDNA.div$qPCR.match) == 4))
reps.dis.phy <- rep(NA, length(reps.phy))

for(i in 1:length(reps.phy)){
  temp <- eDNA.div[eDNA.div$qPCR.match == reps.phy[i] & eDNA.div$treat == "C", ]
  temp.names <- row.names(temp)
  reps.dis.phy[i] <- eDNA.phylo.m[temp.names[1], temp.names[2]]
}

mean(reps.dis.phy)
sem(reps.dis.phy)

t.test(pair.dis.phy, reps.dis.phy)
```
...
# Calculate CVs of taxonomic and phylogentic centroid ratios
feces.cent.tax.CV <- sd(feces.cent.tax$ratio)/mean(feces.cent.tax$ratio)
sed.cent.tax.CV <- sd(sed.cent.tax$ratio)/mean(sed.cent.tax$ratio)
soil.cent.tax.CV <- sd(soil.cent.tax$ratio)/mean(soil.cent.tax$ratio)
water.cent.tax.CV <- sd(water.cent.tax$ratio)/mean(water.cent.tax$ratio)

feces.cent.phy.CV <- sd(feces.cent.phy$ratio)/mean(feces.cent.phy$ratio)
sed.cent.phy.CV <- sd(sed.cent.phy$ratio)/mean(sed.cent.phy$ratio)
soil.cent.phy.CV <- sd(soil.cent.phy$ratio)/mean(soil.cent.phy$ratio)
water.cent.phy.CV <- sd(water.cent.phy$ratio)/mean(water.cent.phy$ratio)
```
...
text("a", x = 0.6, y = 1.475, font = 2, cex = 1.25)
...
text("b", x = 0.6, y = 1.475, font = 2, cex = 1.25)
...
```{r}
cent.dist.tax.ord <- centroid.distances.tax[
  order(centroid.distances.tax$qPCR.match), ]

cent.tax.reg.full <- lm(cent.dist.tax.ord$ratio ~ eDNA.prop.sub2$env + 
                          eDNA.prop.sub2$prop + 
                          eDNA.prop.sub2$env*eDNA.prop.sub2$prop)
cent.tax.reg <- lm(cent.dist.tax.ord$ratio ~ prop, data = eDNA.prop.sub2)

cent.dist.phy.ord <- centroid.distances.phy[
  order(centroid.distances.phy$qPCR.match), ]

cent.phy.reg.full <- lm(cent.dist.phy.ord$ratio ~ eDNA.prop.sub2$env + 
                          eDNA.prop.sub2$prop +
                          eDNA.prop.sub2$env*eDNA.prop.sub2$prop)
cent.phy.reg <- lm(cent.dist.phy.ord$ratio ~ prop, data = eDNA.prop.sub2)

summary(cent.tax.reg.full)
summary(cent.phy.reg.full)
```
...
layout(matrix(c(1:2), byrow = T)) #, heights = c(0.45, 0.45, 0.05))
par(mar = c(1, 6.5, 0.5, 0.5), oma = c(7, 2, 0.5, 0.5))

plot(eDNA.prop.sub2$prop, centroid.distances.tax[match(centroid.distances.tax$qPCR.match, eDNA.prop.sub2$sample.number), ]$ratio,
    xlim = c(0, 1), ylim = c(0.5, 1.5),
     pch = 24, bg = "NA", col = "NA", lwd = 2, bty = "n",
     cex = 2, yaxt = "n", xaxt = "n", cex.lab = 2, cex.axis = 1.5,
     las = 1, ylab = "", xlab = "")
...
col.pt <- rep(NA, length(eDNA.prop.sub2$prop))
#col.bg <- rep(NA, length(eDNA.prop.sub2$prop))
pch.pt <- rep(NA, length(eDNA.prop.sub2$prop))
for(i in 1:length(col.pt)){
  if(eDNA.prop.sub2$env[i] == "soil"){col.pt[i] = "brown";  pch.pt[i] = 24}
  if(eDNA.prop.sub2$env[i] == "sed"){col.pt[i] = "darkgreen"; pch.pt[i] = 22}
  if(eDNA.prop.sub2$env[i] == "feces"){col.pt[i] = "red"; pch.pt[i] = 21}
  if(eDNA.prop.sub2$env[i] == "water"){col.pt[i] = "blue"; pch.pt[i] = 23}
}

# points(eDNA.prop.sub2$prop,
#        centroid.distances.tax[match(centroid.distances.tax$qPCR.match,
#                                     eDNA.prop.sub2$sample.number), ]$ratio, 
#        pch = pch.pt, col = col.pt, bg = col.pt, lwd = 2, cex = 1.7)
...
points(eDNA.prop.sub2$prop[which(eDNA.prop.sub2$env == "soil")], centroid.distances.tax[order(centroid.distances.tax$qPCR.match), ]$ratio[which(centroid.distances.tax[order(centroid.distances.tax$qPCR.match),]$env == "soil")], pch = 24, 
       col = "brown", bg = "brown", lwd = 2, cex = 1.5)
...
points(eDNA.prop.sub2$prop[which(eDNA.prop.sub2$env == "sed")], centroid.distances.tax[order(centroid.distances.tax$qPCR.match), ]$ratio[which(centroid.distances.tax[order(centroid.distances.tax$qPCR.match),]$env == "sed")], pch = 22, 
       col = "darkgreen", bg = "darkgreen", lwd = 2, cex = 1.5)
...
points(eDNA.prop.sub2$prop[which(eDNA.prop.sub2$env == "water")], centroid.distances.tax[order(centroid.distances.tax$qPCR.match), ]$ratio[which(centroid.distances.tax[order(centroid.distances.tax$qPCR.match),]$env == "water")], pch = 23, 
       col = "blue", bg = "blue", lwd = 2, cex = 1.5)

text("a", x = 0.02, y = 1.45, font = 2, cex = 1.25)

text(x = 0.5, y = 1.45, cex = 0.7, pos = 4,
     bquote("Slope:" ~ italic("P") ~ "=" ~
              .(round(ttest(cent.tax.reg, 2, 0)[3], 2))))
text(x = 0.5, y = 1.375, cex = 0.7, pos = 4,
     bquote("Intercept:" ~ italic("P") ~ "=" ~
...
# Legend
legend(0.1, 0.1, c("soil", "sediment", "gut", "water"), 
       pch = c(24, 22, 21, 23), xpd = NA, 
       pt.bg = c("brown", "darkgreen", "red", "blue"), pt.cex = 1.5, 
       pt.lwd = 2, bty = 'n', ncol = 2)
...
# Centroid Distance Ratio PCoA Plots
```{r}
# Data Check
all.equal(row.names(eDNA.div), row.names(OTU.REL.log))
all.equal(row.names(eDNA.div), row.names(as.matrix(phylo.dist)))

centroid.dist.pcoa <- function(env = "" , sbys = "", 
                               design = "", phylo = NULL){
  # Organize Info
  info <- design[which(design$env == env), ]
  sbys2 <- sbys[which(row.names(sbys) %in% row.names(info)), ]
  if(is.null(phylo)){
    ...
    # Calculate Centroid
    disp <- betadisper(bc.dist, rep(1, dim(sbys2)[1]))
    names.vectors <- row.names(disp$vectors)
    vectors <- disp$vectors
    centroids <- disp$centroids[1, ]
  } else {
    phylo2 <- as.matrix(phylo.dist)
    phylo3 <- as.dist(phylo2[which(rownames(phylo2) %in% row.names(info)),
                     which(colnames(phylo2) %in% row.names(info))])
  ...
  line.type <- rep(NA, length(group))
  for(i in 1:length(line.type)){
    if(group[i] == "C"){line.type[i] = 1}
    if(group[i] == "E"){line.type[i] = 2}
  }
  
  line.col <- rep(NA, length(group))
  for(i in 1:length(line.type)){
    if(group[i] == "C"){line.col[i] = "red"}
    if(group[i] == "E"){line.col[i] = "blue"}
  }
  
  vec.col <- rep(NA, length(group))
  for(i in 1:length(line.type)){
    if(group[i] == "C"){vec.col[i] = "white"}
    if(group[i] == "E"){vec.col[i] = "peachpuff"}
  }
  
  return(list(vectors = vectors, names.vectors = names.vectors,  
              centroids = centroids, 
              line.type = line.type, line.col = line.col, vec.col = vec.col))
}


# Calculate Taxonomic Centroid Distances and Table
soil.cent.tax.p <- centroid.dist.pcoa(env = "soil" , sbys = OTU.REL.log, 
                                      design = eDNA.div)
sed.cent.tax.p <- centroid.dist.pcoa(env = "sed" , sbys = OTU.REL.log, 
                                     design = eDNA.div)
feces.cent.tax.p <- centroid.dist.pcoa(env = "feces" , sbys = OTU.REL.log,
                                       design = eDNA.div)
water.cent.tax.p <- centroid.dist.pcoa(env = "water" , sbys = OTU.REL.log,
                                       design = eDNA.div)

png(filename="../figures/FigureS5-CentroidDist.png",
    width = 900, height = 900, res = 96*2)

layout(matrix(c(1:4), byrow = T, ncol = 2))
par(mar = c(0.5, 0.5, 0.5, 0.5), oma = c(3, 3, 0.5, 0.5))

# Soil
plot(0, 0, type = 'n', xlim = c(-0.6, 0.6), ylim = c(-0.6, 0.6), las = 1,
     xlab = "PCoA 1", ylab = "PCoA 2", axes = F)
abline(h = 0, lty = 3);abline(v = 0, lty = 3); box(lwd = 1.5)
text(x=-0.6, y = 0.5, "Soil", pos = 4)
points(soil.cent.tax.p$centroids[1], soil.cent.tax.p$centroids[2], 
       pch = 21, bg = "black")
points(jitter(soil.cent.tax.p$vectors[, 1]),
       jitter(soil.cent.tax.p$vectors[, 2]),
       pch = 22, cex = 1.5, bg = soil.cent.tax.p$vec.col)
arrows(x0 = soil.cent.tax.p$centroids[1], 
       y0 = soil.cent.tax.p$centroids[2],
       x1 = soil.cent.tax.p$vectors[, 1],
       y1 = soil.cent.tax.p$vectors[, 2],
       length = 0, lwd = 1.5, 
       lty = soil.cent.tax.p$line.type, col = soil.cent.tax.p$line.col)
# Sediment
plot(0, 0, type = 'n', xlim = c(-0.6, 0.6), ylim = c(-0.6, 0.6), las = 1,
     xlab = "PCoA 1", ylab = "PCoA 2", axes = F)
abline(h = 0, lty = 3);abline(v = 0, lty = 3); box(lwd = 1.5)
text(x=-0.6, y = 0.5, "Sediments", pos = 4)
points(sed.cent.tax.p$centroids[1], sed.cent.tax.p$centroids[2], 
       pch = 21, bg = "black")
points(jitter(sed.cent.tax.p$vectors[, 1]),
       jitter(sed.cent.tax.p$vectors[, 2]),
       pch = 22, cex = 1.5, bg = sed.cent.tax.p$vec.col)
arrows(x0 = sed.cent.tax.p$centroids[1], 
       y0 = sed.cent.tax.p$centroids[2],
       x1 = sed.cent.tax.p$vectors[, 1],
       y1 = sed.cent.tax.p$vectors[, 2],
       length = 0, lwd = 1.5, 
       lty = sed.cent.tax.p$line.type, col = sed.cent.tax.p$line.col)
# Feces
plot(0, 0, type = 'n', xlim = c(-0.6, 0.6), ylim = c(-0.6, 0.6), las = 1,
     xlab = "PCoA 1", ylab = "PCoA 2", axes = F)
abline(h = 0, lty = 3);abline(v = 0, lty = 3); box(lwd = 1.5)
text(x=-0.6, y = 0.5, "Feces", pos = 4)
points(feces.cent.tax.p$centroids[1], feces.cent.tax.p$centroids[2], 
       pch = 21, bg = "black")
points(jitter(feces.cent.tax.p$vectors[, 1]),
       jitter(feces.cent.tax.p$vectors[, 2]),
       pch = 22, cex = 1.5, bg = feces.cent.tax.p$vec.col)
arrows(x0 = feces.cent.tax.p$centroids[1], 
       y0 = feces.cent.tax.p$centroids[2],
       x1 = feces.cent.tax.p$vectors[, 1],
       y1 = feces.cent.tax.p$vectors[, 2],
       length = 0, lwd = 1.5, 
       lty = feces.cent.tax.p$line.type, col = feces.cent.tax.p$line.col)
# Water
plot(0, 0, type = 'n', xlim = c(-0.6, 0.6), ylim = c(-0.6, 0.6), las = 1,
     xlab = "PCoA 1", ylab = "PCoA 2", axes = F)
abline(h = 0, lty = 3);abline(v = 0, lty = 3); box(lwd = 1.5)
text(x=-0.6, y = 0.5, "Water", pos = 4)
points(water.cent.tax.p$centroids[1], water.cent.tax.p$centroids[2], 
       pch = 21, bg = "black")
points(jitter(water.cent.tax.p$vectors[, 1]),
       jitter(water.cent.tax.p$vectors[, 2]),
       pch = 22, cex = 1.5, bg = water.cent.tax.p$vec.col)
arrows(x0 = water.cent.tax.p$centroids[1], 
       y0 = water.cent.tax.p$centroids[2],
       x1 = water.cent.tax.p$vectors[, 1],
       y1 = water.cent.tax.p$vectors[, 2],
       length = 0, lwd = 1.5, 
       lty = water.cent.tax.p$line.type, col = water.cent.tax.p$line.col)
...
# Show Plot
img <- readPNG("../figures/FigureS5-CentroidDist.png")
grid.raster(img)
```
...
OTU2 <- rrarefy(OTU, 30000)

OTU2.REL <- OTU2
for (i in 1:dim(OTU2)[1]){
  OTU2.REL[i,] <- OTU2[i,]/sum(OTU2[i,])
  }

# Check order of design
all.equal(row.names(eDNA.div), row.names(OTU2.REL))

# Subset OTU Matrix for Each Molecule
OTU2.REL.c <- OTU2.REL[which(eDNA.div$treat == "C"), ]
OTU2.REL.e <- OTU2.REL[which(eDNA.div$treat == "E"), ]
...
# Make Sure Subset OTU Matrices are Aligned by Sample Code
OTU2.REL.c2 <- OTU2.REL.c[order(eDNA.div.c$samp.code), ]
OTU2.REL.e2 <- OTU2.REL.e[order(eDNA.div.e$samp.code), ]
eDNA.div.c2 <- eDNA.div.c[order(eDNA.div.c$samp.code), ]
eDNA.div.e2 <- eDNA.div.e[order(eDNA.div.e$samp.code), ]

all.equal(row.names(OTU2.REL.c2), row.names(eDNA.div.c2))
all.equal(row.names(OTU2.REL.e2), row.names(eDNA.div.e2))

dim(OTU2.REL.c2); dim(OTU2.REL.e2)

# Make Commulative Rel Abunadnce 
rowSums(OTU2.REL.c2); rowSums(OTU2.REL.e2)

OTU2.RELc.c2 <- t(apply(OTU2.REL.c2, 1, cumsum))
OTU2.RELc.e2 <- t(apply(OTU2.REL.e2, 1, cumsum))

ks.test(OTU2.RELc.c2[1, ], OTU2.RELc.e2[1, ], exact = T)

plot(OTU2.RELc.c2[i, 1:1000], cex = 0.5, col = "red", ylim = c(0, 1))
points(OTU2.RELc.e2[i, 1:1000], cex = 0.5, col = "blue", ylim = c(0, 1))

plot(log10(OTU2.RELc.c2[i, 1:100]), cex = 0.5, col = "red", ylim = c(-1, 0))
points(log10(OTU2.RELc.e2[i, 1:100]), cex = 0.5, col = "blue", ylim = c(-1, 0))

ks.stats <- as.data.frame(matrix(NA, nrow = dim(OTU2.RELc.c2)[1], ncol = 5))
colnames(ks.stats) <- c("Env", "Pair", "Samp_Code", "D", "p")

for(i in 1:dim(OTU.RELc.c2)[1]){
  ks.temp <- suppressWarnings(
               ks.test((OTU2.REL.c2[i,]), 
                       (OTU2.REL.e2[i, ]), exact = T))
  ks.stats[i, 1] <- as.character(eDNA.div.c2$env[i])
  ks.stats[i, 2] <- eDNA.div.c2$pair[i]
  ks.stats[i, 3] <- as.character(eDNA.div.c2$samp.code[i])
  ks.stats[i, 4] <- ks.temp$statistic
  ks.stats[i, 5] <- round(ks.temp$p.value, 3)
}

aggregate(p ~ Env, data = ks.stats, mean)
aggregate(p ~ Env, data = ks.stats, sem)
aggregate(p ~ Env, data = ks.stats, min)
