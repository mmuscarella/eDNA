...
plot(log10(active1), col = "gray",
     ...
     ylim = c(0, max(max(log10(active1)), max(log10(relic2)))),
     las = 1, type = "p", xaxt = "n", yaxt = "n", pch = 16)
points(log10(relic1), col = "gray30", type = "p", pch = 15)
points(log10(relic2), col = "gray30", type = "p", pch = 16)
points(log10(relic3), col = "gray30", type = "p", pch = 17)
legend("topright", legend = c("Active", "Relic Same Rates", 
       "Relic High Decay", "Relic Low Decay"), pch = c(16, 15, 16, 17), 
...
relic2.prob <- relic2 / sum(relic2)    # High
relic3.prob <- relic3 / sum(relic3)    # Low

max(length(active1), length(relic1), length(relic2), length(relic3))

# Define OTUs
otus <- paste("OTU", sprintf("%05d", seq(1:100000)), sep = "")

gamma1 <- rlnorm(n=10000, meanlog = 1, sdlog = 0.98)
gamma1 <- gamma1[rev(order(gamma1))]

# Initiate Communities
site1 <- sample(names(active1), size = length(active1), replace = T, prob = active1)
site2 <- sample(names(active1), size = length(active1), replace = T, prob = active1)
site3 <- sample(names(active1), size = length(active1), replace = T, prob = active1)
...
ratio.tab <- matrix(NA, nrow = 5, ncol = length(props))
rownames(ratio.tab) <- c("Null", "Same", "High", "Low", "Proportion")
...
for(i in 1:length(props)){
  ...
  temp.null <- c(sample(names(active1), size = size.comm, 
                        replace = T, prob = active1))
  
  temp.same <- c(sample(names(active1), size = size.comm, 
                          replace = T, prob = active1),
                   sample(names(relic1), size = n, 
                          replace = T, prob = relic1))
  
  temp.high <- c(sample(names(active1), size = size.comm, 
                        replace = T, prob = active1),
                 sample(names(relic2), size = n, 
                        replace = T, prob = relic2))
  
  temp.low <- c(sample(names(active1), size = size.comm, 
                        replace = T, prob = active1),
                 sample(names(relic3), size = n, 
                        replace = T, prob = relic3))
  
  tab1 <- table(temp.null)
  tab2 <- table(temp.same)
  tab3 <- table(temp.high)
  tab4 <- table(temp.low)
