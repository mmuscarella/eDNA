...
# Same Rates
total1 <- table(sample(c(same.rates[[1]], same.rates[[2]]), 10000))
active1 <- table(sample(same.rates[[1]], 10000))
relic1 <- table(sample(same.rates[[2]], 10000))
...
# Low Decay
total2 <- table(sample(c(low.decay[[1]], low.decay[[2]]), 10000))
active2 <- table(sample(low.decay[[1]], 10000))
relic2 <- table(sample(low.decay[[2]], 10000))
...
# High Decay
total3 <- table(sample(c(high.decay[[1]], high.decay[[2]]), 10000))
active3 <- table(sample(high.decay[[1]], 10000))
relic3 <- table(sample(high.decay[[2]], 10000))
...
points(log10(relic2), col = "gray30", type = "p", pch = 16)
...
size.comm <- 10000
n.relic <- seq(size.comm/100, size.comm, 500)
n.active <- size.comm - n.relic
props <- n.relic/size.comm

ratio.tab <- matrix(NA, nrow = 5, ncol = length(props))
rownames(ratio.tab) <- c("Null", "Same", "Low", "High", "Proportion")
...
for(i in 1:length(props)){
  ...
  # Simuate Mixing Mdel
  temp.null <- c(sample(active1.n, size = size.comm, 
                        replace = T, prob = active1))
  
  temp.same <- c(sample(active1.n, size = n1, 
                          replace = T, prob = active1/sum(active1)),
                   sample(relic1.n, size = n2, 
                          replace = T, prob = relic1/sum(relic1)))
  
  temp.low <- c(sample(active2.n, size = n1, 
                        replace = T, prob = active2/sum(active2)),
                 sample(relic2.n, size = n2, 
                        replace = T, prob = relic2/sum(relic2)))
  
  temp.high <- c(sample(active3.n, size = n1, 
                        replace = T, prob = active3/sum(active3)),
                 sample(relic3.n, size = n2, 
                        replace = T, prob = relic3/sum(relic3)))
  
  tab1 <- table(temp.null)
  tab2 <- table(temp.same)
  tab3 <- table(temp.low)
  tab4 <- table(temp.high)

  ratio.tab[1, i] <- round(length(tab1)/length(tab1), 2)
  ratio.tab[2, i] <- round(length(tab2)/length(tab1), 2)
  ratio.tab[3, i] <- round(length(tab3)/length(tab1), 2)
  ratio.tab[4, i] <- round(length(tab4)/length(tab1), 2)
  ratio.tab[5, i] <- props[i]
